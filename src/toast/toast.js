'use strict';
  ...
  var inApp = navigator.userAgent.match(/WindVane/);

  function Toast(text, disappearTime, nativeFirst) {
      var self = this;

      text = text || '';
      disappearTime = disappearTime || 3500;
      if (nativeFirst === undefined) {
          nativeFirst = true;
      }

      if (nativeFirst && inApp && Ali && Ali.ready && Ali.toast) {
          Ali.ready(function (Ali) {
              Ali.toast(text, function () {
              });
          });
